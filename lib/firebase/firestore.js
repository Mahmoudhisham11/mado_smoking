...
// Simple cache for stores to avoid repeated fetches
...
export const getStore = async (storeId) => {
  try {
    ...
    if (storeSnap.exists()) {
      ...
      // Update cache
      storeCache.set(storeId, { data: storeData, timestamp: Date.now() });
      return storeData;
    }
    return null;
  } catch (error) {
...
// Helper function to clear store cache (useful after updates)
export const clearStoreCache = (storeId = null) => {
  if (storeId) {
...
export const subscribeToInvoices = (sourceId, callback) => {
  try {
    ...
    return onSnapshot(q, async (snapshot) => {
      ...
      // Fetch store names for invoices - optimized with Promise.all
      const uniqueStoreIds = [...new Set(invoices.map(inv => inv.storeId).filter(Boolean))];
      if (uniqueStoreIds.length > 0) {
        try {
          // Fetch all stores in parallel
          const storesPromises = uniqueStoreIds.map(storeId => getStore(storeId));
          const stores = await Promise.all(storesPromises);
          // Create a map for fast lookup
          const storesMap = new Map(stores.filter(Boolean).map(store => [store.id, store]));
          // Assign storeName to each invoice
          invoices.forEach(invoice => {
            if (invoice.storeId && !invoice.storeName) {
              const store = storesMap.get(invoice.storeId);
...
export const returnInvoice = async (invoiceId) => {
  try {
    ...
    // في حالة المرتجع الكامل، نحذف جميع المنتجات التي لها نفس code بغض النظر عن المخزن
    if (invoiceData.products && invoiceData.products.length > 0) {
      for (const product of invoiceData.products) {
        ...
        // حذف جميع المنتجات التي لها نفس code من كل المخازن
...
export const addPayment = async (sourceId, paymentAmount, cashRegisterId) => {
  try {
    ...
    // Allow payment even if balance is insufficient - balance can go negative
    // No need to check balance before payment
...
export const returnProduct = async (invoiceId, productIndex) => {
  try {
    ...
    const newTotalCost = updatedProducts.reduce((sum, p) => sum + ((p.wholesalePrice || 0) * (p.quantity || 0)), 0);
...
export const addCustomerInvoice = async (invoiceData) => {
  try {
    ...
    // Note: Payment is handled in handleAddInvoice in the page component
    // to ensure it's added to the correct cash register selected by the user
...
export const returnCustomerInvoice = async (invoiceId) => {
  try {
    ...
    // في حالة المرتجع الكامل، حذف الفاتورة بالكامل بعد إرجاع المنتجات
    await deleteDoc(invoiceRef);
...
export const deleteCustomerPayment = async (paymentId) => {
  try {
    ...
    // Allow negative balance - no need to check balance before deletion
    if (cashRegisterId) {
      ...
      // Allow deletion even if balance would go negative
      await updateCashRegisterBalance(cashRegisterId, -paymentAmount);
